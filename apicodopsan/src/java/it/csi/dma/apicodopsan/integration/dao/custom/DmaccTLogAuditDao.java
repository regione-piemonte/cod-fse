...
import it.csi.dma.apicodopsan.integration.auditlog.DmaccTLogAuditSupportForInsert;
...
//			+ "(select cat.descrizione from dmacc_d_catalogo_log_audit cat where cat.codice=:codiceAudit ), "
...
//			+ "(select cat2.id from dmacc_d_catalogo_log_audit cat2 where cat2.codice=:codiceAudit ), "
			+ ":idCatalogoLogAudit, "
			+ "now(), "
			+ ":xRequestId, "
			...
			+ ":codice_fiscale_assistito "
			+ ");";
	//RIGA 41 per cittadino 	sostituire		+ "(select druolo.id from dmacc_d_ruolo druolo where druolo.codice_ruolo=(SELECT x.codice_ruolo_fse FROM cod.dmacc_t_decodifica_ruoli_pua x WHERE codice_ruolo_pua =:ruoloIn and sol=:xcodiceVerticale)),"
	//select ddr.id from dmacc_d_ruolo ddr where ddr.codice_ruolo =:ruoloIn;
	@Autowired
