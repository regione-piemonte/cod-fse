...
@Component
public class DisabilitaPazientiCittadinoNotificaAsync extends LoggerUtil {
	...
	@Async
	public void notifyAsync(String shibIdentitaCodiceFiscale, List<Soggetto> pazientiList, String xRequestId,
		...
		if (pazientiList != null && pazientiList.size() > 0) {
			...
			for (Soggetto soggettoRicevente : pazientiList) {
				try {
					notificatoreService.notificaEventoDisabilitaPaziente(shibIdentitaCodiceFiscale, soggettoRicevente,
							xRequestId);
					...
					if (listaDelegati != null && listaDelegati.size() > 0) {
						for (Soggetto delegato : listaDelegati) {
							notificatoreService.notificaEventoDisabilitaPazientePerDelegato(shibIdentitaCodiceFiscale,
									delegato, soggettoRicevente.getSoggettoCf(), xRequestId);
						}
					}
					sb.append("OK");
			...
			logInfo("notifyAsync", "lista pazienti disabilitati vuota");
		}
