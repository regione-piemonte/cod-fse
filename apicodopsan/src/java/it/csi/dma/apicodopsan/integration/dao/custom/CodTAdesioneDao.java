...
import it.csi.dma.apicodopsan.integration.dao.utils.TAdesioneMapper;
...
public static final String SELECT_ADESIONE =	        "SELECT 									" +
												        ...
												        "	AND CTA.ADESIONE_INIZIO <= now() " +
												        "	AND (CTA.ADESIONE_FINE >= now() 	" +
												        "		OR CTA.ADESIONE_FINE IS NULL) 		" +
												        "ORDER BY CTA.ADESIONE_ID DESC LIMIT 1      ";
public static final String SELECT_ULTIMA_ADESIONE =	        "SELECT 									" +
													        "CTA.* 										" +
													        "FROM 										" +
													        "	COD_T_ADESIONE CTA, 					" +
													        "	COD_T_SOGGETTO CTS 						" +
...
public TAdesione selectUltimaAdesione(String cfSoggetto) throws DatabaseException {
		...
		try {
			TAdesione selected = jdbcTemplate.queryForObject(SELECT_ULTIMA_ADESIONE, namedParameters, new TAdesioneMapper());
			return selected;
		} catch (EmptyResultDataAccessException e) {
			return null;
		} catch (Exception e) {
