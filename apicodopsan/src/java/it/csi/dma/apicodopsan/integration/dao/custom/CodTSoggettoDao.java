...
import it.csi.dma.apicodopsan.integration.dao.utils.AssistitiAbilitatiMapper;
import it.csi.dma.apicodopsan.integration.dao.utils.AssistitiDisabilitatiMapper;
import it.csi.dma.apicodopsan.integration.dao.utils.CountSoggettiMapper;
import it.csi.dma.apicodopsan.integration.dao.utils.SoggettoMapper;
...
public static final String SELECT_COUNT_SOGGETTI_ABILITATI_DISABILITATI = "SELECT MAX(countAbilitato) AS abilitati, MAX(countDisabilitato) as disabilitati FROM ( "
																	...
																	+ "	FROM cod_t_soggetto_abilitato ctsa "
...
private static final String BASE_FINALE_CHIUSURA = "; ";
	...
	//26/10/2022
	private static final String UPDATE_CITTADINO_TO_MEDICO="UPDATE cod_t_soggetto SET soggetto_is_medico=true, data_modifca=now(), utente_modifica=:codiceFiscale WHERE soggetto_id=:soggettoId";
...
//			sb.append("AND CTS.SOGGETTO_IS_MEDICO = false");
	...
	public void updateCittadinoToMedico(String codiceFiscale,Integer soggettoId) throws DatabaseException{
		var methodName = "updateCittadinoToMedico";
		SqlParameterSource namedParameters = new MapSqlParameterSource().addValue("codiceFiscale", codiceFiscale).addValue("soggettoId", soggettoId);
		try {
			jdbcTemplate.update(UPDATE_CITTADINO_TO_MEDICO, namedParameters);
		} catch (EmptyResultDataAccessException e) {
			logError(methodName, e.getMessage());
			throw new DatabaseException(e);
		} catch (Exception e) {
