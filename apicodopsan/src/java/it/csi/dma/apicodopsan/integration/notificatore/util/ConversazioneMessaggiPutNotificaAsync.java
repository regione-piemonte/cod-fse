...
public class ConversazioneMessaggiPutNotificaAsync  extends LoggerUtil{
	...
	public void notifyAsync(String shibIdentitaCodiceFiscale,Integer soggettoAutore, String xRequestId,String xCodiceServizio)  {
		...
		try {
			...
			notificatoreService.notificaEventoMessaggioPut(shibIdentitaCodiceFiscale, soggetto,xRequestId);
			...
			if(listaDelegati!=null && listaDelegati.size()>0) {
				for(Soggetto delegato:listaDelegati) {
					notificatoreService.notificaEventoMessaggioPutPerDelegato(shibIdentitaCodiceFiscale, delegato, soggetto.getSoggettoCf(),xRequestId);
				}
			}
			sb.append("OK");
