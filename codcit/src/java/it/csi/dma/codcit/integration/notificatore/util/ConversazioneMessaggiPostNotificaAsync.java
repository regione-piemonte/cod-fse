...
public class ConversazioneMessaggiPostNotificaAsync  extends LoggerUtil{
	...
	public void notifyAsync(String citId, Integer medicoId, String xRequestId,
		...
		try {
			...
			// String cfMittente, Soggetto soggettoRicevente, String xRequestId,String
			// xForwardedFor
			Soggetto soggettoMedico = codTSoggettoDao.selectSoggettoById(medicoId);
			notificatoreMedicoService.notificaEventoMessaggioPost(citId, soggettoMedico, xRequestId,
					xForwardedFor);
			if(!shibIdentitaCodiceFiscale.equals(citId)) {
				Soggetto soggettoCittadino = codTSoggettoDao.selectSoggettoByCF(citId);
				notificatoreService.notificaEventoMessaggioPost(shibIdentitaCodiceFiscale, soggettoCittadino,xRequestId);
				isDelegato=true;
			}
			...
			if (listaDelegati != null && listaDelegati.size() > 0) {
				for (Soggetto delegato : listaDelegati) {
					if(!(isDelegato && delegato.getSoggettoCf().equals(shibIdentitaCodiceFiscale))) {
						notificatoreService.notificaEventoMessaggioPost(citId, delegato,xRequestId);
					}
